...
- **Build/test:** needs Windows + .NET 8 SDK (`WindowsBlocker.sln`); not buildable on the Mac. JS contracts in `tests/` run here: `runner-custom-rule-stress.js` via macOS `jsc`, the other two via `node`.
